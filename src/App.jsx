...
api.interceptors.response.use(
  ...
  async (error) => {
    ...
    if (response?.status === 401 && !originalRequest._retry) {
      ...
      try {
        // refresh_token 값은 HttpOnly 쿠키로 전달되므로 본문에는 포함하지 않습니다.
        // (withCredentials: true 로 쿠키 자동 동봉)
...
public class SecurityConfig {
    ...
    public SecurityFilterChain securityFilterChain(HttpSecurity http) throws Exception {
        http
            // 1. MSA 서비스 특성에 따른 상태 비저장(Stateless) 세션 정책 구성
            ...
            // 5. JWT 기반 인가 서버(Issuer) 디코더 연동 설정
            .oauth2ResourceServer(oauth2 -> oauth2
                .jwt(jwt -> jwt
                    // 디지털플랫폼 통합인증 서버가 노출해둔 공개 키 집합 경로(JWKS) 지정
                    .jwkSetUri("https://auth.kwater.com/oauth2/v1/jwks")
                )
            );
