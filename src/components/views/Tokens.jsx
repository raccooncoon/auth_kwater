  ...
  return (
    <div className="bg-slate-900 border border-slate-800 rounded-2xl overflow-hidden">
      ...
      >
        ...
        <span className="shrink-0 text-[13px] font-mono font-bold uppercase tracking-widest text-slate-500 mr-1">
          {open ? '접기' : '펼치기'}
        </span>
        <ChevronDown
    ...
    <div className="space-y-8">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        <div className="bg-slate-950/40 p-6 rounded-2xl border border-indigo-500/20 relative overflow-hidden">
          <div className="absolute top-0 right-0 bg-indigo-500/10 text-indigo-400 text-[14px] font-mono px-3 py-1 rounded-bl-xl border-l border-b border-indigo-500/10">Access Token</div>
        ...
        <div className="bg-slate-950/40 p-6 rounded-2xl border border-emerald-500/20 relative overflow-hidden">
          <div className="absolute top-0 right-0 bg-emerald-500/10 text-emerald-400 text-[14px] font-mono px-3 py-1 rounded-bl-xl border-l border-b border-emerald-500/10">Refresh Token</div>
          <h3 className="text-lg font-bold text-white mb-2 flex items-center gap-2">
      ...
      >
        <div className="p-6 space-y-4">
          ...
          <pre className="text-[15px] bg-slate-950 border border-slate-800 rounded-lg p-4 font-mono overflow-x-auto whitespace-pre leading-relaxed">
<span className="text-indigo-300">eyJhbGciOiJSUzI1NiIsInR5cCI6IkpXVCIsImtpZCI6IjEyMyJ9</span>.<span className="text-emerald-300">eyJpc3MiOiJodHRwczovL2F1dGgua3dhdGVyLmNvbSIsInN1YiI6Imt3YXRlcl91c2VyXzEyMzQiLCJhdWQiOiJjbXAtcG9ydGFsIiwic2NvcGUiOiJvcGVuaWQgcHJvZmlsZSIsImV4cCI6MTcxNjk4NTU5MywiaWF0IjoxNzE2OTgxOTkzfQ</span>.<span className="text-rose-300">[RSA Signature]</span></pre>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
            <div className="bg-slate-950/60 border border-indigo-500/30 rounded-xl p-4">
              <div className="text-indigo-300 text-[14px] font-mono font-bold uppercase mb-2">Header (헤더)</div>
              <CodeBlock language="json" fontSize="0.72rem" code={`{
  "alg": "RS256",
  "typ": "JWT",
  "kid": "123"
}`} />
              <p className="text-[15px] text-slate-400 mt-2">서명 알고리즘과 키 ID. 리소스 서버가 JWKS에서 어떤 공개키로 검증할지 식별.</p>
            </div>
            <div className="bg-slate-950/60 border border-emerald-500/30 rounded-xl p-4">
              <div className="text-emerald-300 text-[14px] font-mono font-bold uppercase mb-2">Payload (페이로드)</div>
              <CodeBlock language="json" fontSize="0.72rem" code={`{
  ...
  "iat": 1716981993
              ...
              <p className="text-[15px] text-slate-400 mt-2">사용자 정보·권한·만료시각. <strong className="text-emerald-300">암호화되지 않음</strong> — 비밀번호 절대 금지.</p>
            </div>
            <div className="bg-slate-950/60 border border-rose-500/30 rounded-xl p-4">
              <div className="text-rose-300 text-[14px] font-mono font-bold uppercase mb-2">Signature (서명)</div>
              <CodeBlock language="javascript" fontSize="0.72rem" code={`RSA_SHA256(
  Base64(header) + "." +
  Base64(payload),
  IdP_PrivateKey
)`} />
              <p className="text-[15px] text-slate-400 mt-2">IdP의 비공개키로 서명. 페이로드를 1바이트만 바꿔도 서명 검증 실패.</p>
            </div>
          </div>
          ...
          <div className="bg-slate-950 border border-slate-800 rounded-xl p-5 mt-2">
            ...
            <p className="text-[15px] text-slate-400 leading-relaxed mb-3">
              위 토큰의 <strong className="text-indigo-300">Header 부분</strong>(<code className="text-[13px] bg-slate-900 px-1.5 py-0.5 rounded text-indigo-300">eyJhbGc...</code>)을 직접 디코딩해봅시다.
            </p>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
              <div>
                <div className="text-[14px] font-mono text-slate-500 mb-1.5">① 입력 — Base64URL 문자열</div>
                <pre className="text-[14px] bg-slate-900 border border-slate-800 rounded p-3 text-indigo-300 font-mono overflow-x-auto">eyJhbGciOiJSUzI1NiIsInR5cCI6IkpXVCIsImtpZCI6IjEyMyJ9</pre>
              </div>
              <div>
                <div className="text-[14px] font-mono text-slate-500 mb-1.5">② 디코딩 결과 — JSON</div>
                <pre className="text-[14px] bg-slate-900 border border-slate-800 rounded p-3 text-emerald-300 font-mono overflow-x-auto whitespace-pre">{`{
  "alg":"RS256",
  "typ":"JWT",
  "kid":"123"
}`}</pre>
            ...
            <div className="mt-3 text-[14px] text-slate-500 font-mono">
          ...
          <div className="bg-slate-950 border border-slate-800 rounded-xl p-5">
            ...
            <div className="overflow-x-auto">
              <table className="w-full text-left text-[15px] border-collapse">
                <thead>
                  <tr className="border-b border-slate-800 text-slate-500 text-[14px]">
                ...
                <tbody className="divide-y divide-slate-800/60 text-slate-300">
                  <tr><td className="py-2 px-3 font-mono text-emerald-300">iss</td><td className="py-2 px-3">Issuer — 토큰을 발급한 IdP</td><td className="py-2 px-3 font-mono text-[14px] text-slate-400">"https://auth.kwater.com"</td></tr>
                  <tr><td className="py-2 px-3 font-mono text-emerald-300">sub</td><td className="py-2 px-3">Subject — 사용자 식별자</td><td className="py-2 px-3 font-mono text-[14px] text-slate-400">"kwater_user_1234"</td></tr>
                  <tr><td className="py-2 px-3 font-mono text-emerald-300">aud</td><td className="py-2 px-3">Audience — 토큰 수신 대상 (client_id)</td><td className="py-2 px-3 font-mono text-[14px] text-slate-400">"cmp-portal"</td></tr>
                  <tr><td className="py-2 px-3 font-mono text-emerald-300">exp</td><td className="py-2 px-3">Expiration — UNIX 만료 시각</td><td className="py-2 px-3 font-mono text-[14px] text-slate-400">1716985593 (= 2024-05-29 18:13)</td></tr>
                  <tr><td className="py-2 px-3 font-mono text-emerald-300">iat</td><td className="py-2 px-3">Issued At — 발급 시각</td><td className="py-2 px-3 font-mono text-[14px] text-slate-400">1716981993</td></tr>
                  <tr><td className="py-2 px-3 font-mono text-emerald-300">nbf</td><td className="py-2 px-3">Not Before — 이 시각 이전엔 무효</td><td className="py-2 px-3 font-mono text-[14px] text-slate-400">1716981993</td></tr>
                  <tr><td className="py-2 px-3 font-mono text-emerald-300">jti</td><td className="py-2 px-3">JWT ID — 토큰 고유 식별자 (재사용 차단)</td><td className="py-2 px-3 font-mono text-[14px] text-slate-400">"f47ac10b-58cc-4372"</td></tr>
                  <tr><td className="py-2 px-3 font-mono text-emerald-300">scope</td><td className="py-2 px-3">권한 범위 (OAuth Scope)</td><td className="py-2 px-3 font-mono text-[14px] text-slate-400">"openid profile email"</td></tr>
                  <tr><td className="py-2 px-3 font-mono text-emerald-300">nonce</td><td className="py-2 px-3">Replay 방지용 클라이언트 nonce (ID Token만)</td><td className="py-2 px-3 font-mono text-[14px] text-slate-400">"n-0S6_WzA2Mj"</td></tr>
                  <tr><td className="py-2 px-3 font-mono text-emerald-300">sid</td><td className="py-2 px-3">Session ID — SLO 시 세션 식별용</td><td className="py-2 px-3 font-mono text-[14px] text-slate-400">"sso_sess_892347a8f"</td></tr>
                </tbody>
              </table>
            </div>
          </div>
          ...
          <div className="bg-slate-950 border border-emerald-500/30 rounded-xl p-5">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-3 mb-4">
              <div className="bg-slate-900 border border-slate-800 rounded-lg p-4">
                <div className="text-[14px] font-mono font-bold uppercase tracking-wider text-indigo-300 mb-2">Access Token이 답하는 질문</div>
                <p className="text-[15px] text-slate-300 leading-relaxed">"이 요청자가 <strong className="text-white">이 API를 호출해도 되는가?</strong>"</p>
                <p className="text-[14px] text-slate-500 mt-2">→ 권한 (Authorization)</p>
              </div>
              <div className="bg-slate-900 border border-emerald-500/30 rounded-lg p-4">
                <div className="text-[14px] font-mono font-bold uppercase tracking-wider text-emerald-300 mb-2">ID Token이 답하는 질문</div>
                <p className="text-[15px] text-slate-300 leading-relaxed">"이 사용자는 <strong className="text-white">누구이고 어떻게 인증됐는가?</strong>"</p>
                <p className="text-[14px] text-slate-500 mt-2">→ 신원 (Authentication)</p>
              </div>
            </div>

            <div className="text-[15px] font-bold text-white mb-2">ID Token 페이로드 예시 (디코딩된 JSON)</div>
  ...
  "department": "수자원관리본부"
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mt-4">
              <div>
                <div className="text-[15px] font-bold text-white mb-2">ID Token 처리 순서</div>
                <ol className="space-y-1.5 text-[15px] text-slate-300">
              ...
              <div>
                <div className="text-[15px] font-bold text-white mb-2">왜 별도 토큰인가</div>
            ...
            <div className="bg-rose-500/5 border border-rose-500/30 rounded-lg p-4 mt-4">
              <div className="text-[15px] font-bold text-rose-200 mb-2 flex items-center gap-2">
          ...
          <div className="bg-slate-950 border border-slate-800 rounded-xl p-5">
            ...
            <div className="overflow-x-auto">
              <table className="w-full text-left text-[15px] border-collapse">
                ...
                <tbody className="divide-y divide-slate-800/60 text-slate-300 text-[14px]">
          ...
          <div className="bg-slate-950 border border-slate-800 rounded-xl p-5">
            ...
            <ol className="space-y-2.5 text-[15px] text-slate-300">
              <li className="flex gap-3">
                <span className="shrink-0 w-6 h-6 rounded-full bg-rose-500/20 border border-rose-500/40 flex items-center justify-center text-[13px] font-bold text-rose-300">1</span>
                <div><strong className="text-white">Header에서 kid 추출</strong> — 어떤 키로 서명됐는지 식별. 예: <code className="text-[13px] bg-slate-900 px-1.5 py-0.5 rounded text-amber-300">{`{"kid":"123"}`}</code></div>
              </li>
              <li className="flex gap-3">
                <span className="shrink-0 w-6 h-6 rounded-full bg-rose-500/20 border border-rose-500/40 flex items-center justify-center text-[13px] font-bold text-rose-300">2</span>
                <div><strong className="text-white">JWKS에서 공개키 조회</strong> — <code className="text-[13px] bg-slate-900 px-1.5 py-0.5 rounded text-amber-300">GET https://auth.kwater.com/.well-known/jwks.json</code> (1시간 캐시 권장)</div>
              </li>
              <li className="flex gap-3">
                <span className="shrink-0 w-6 h-6 rounded-full bg-rose-500/20 border border-rose-500/40 flex items-center justify-center text-[13px] font-bold text-rose-300">3</span>
                <div><strong className="text-white">서명 검증</strong> — <code className="text-[13px] bg-slate-900 px-1.5 py-0.5 rounded text-amber-300">RSA_verify(header.payload, signature, publicKey)</code> → true여야 함</div>
              </li>
              <li className="flex gap-3">
                <span className="shrink-0 w-6 h-6 rounded-full bg-rose-500/20 border border-rose-500/40 flex items-center justify-center text-[13px] font-bold text-rose-300">4</span>
                <div><strong className="text-white">클레임 검증</strong> — <code className="text-[13px] bg-slate-900 px-1.5 py-0.5 rounded text-amber-300">iss</code>=기대 IdP · <code className="text-[13px] bg-slate-900 px-1.5 py-0.5 rounded text-amber-300">aud</code>=내 client_id · <code className="text-[13px] bg-slate-900 px-1.5 py-0.5 rounded text-amber-300">exp</code>{' > '}now (클럭 스큐 ±60초 허용) · <code className="text-[13px] bg-slate-900 px-1.5 py-0.5 rounded text-amber-300">nbf</code>{' ≤ '}now</div>
              </li>
              <li className="flex gap-3">
                <span className="shrink-0 w-6 h-6 rounded-full bg-rose-500/20 border border-rose-500/40 flex items-center justify-center text-[13px] font-bold text-rose-300">5</span>
                <div><strong className="text-white">선택: jti 재사용 체크</strong> — Redis에 jti가 이미 있으면 거부 (replay 공격 방어)</div>
              </li>
            </ol>
            <div className="mt-4">
              <div className="text-[14px] font-bold text-slate-400 mb-2 flex items-center gap-2">
...
RS256_verify(token, JWKS_public_key)  // → true/false`} />
      ...
      >
        <div className="p-6 space-y-6">
          ...
          <div className="bg-slate-950 border border-slate-800 rounded-xl p-5">
            <div className="text-[14px] font-mono font-bold uppercase tracking-widest text-slate-500 mb-4 text-center">BFF 패턴 권장 저장 구조</div>
            <div className="grid grid-cols-3 gap-4">
              {/* Browser layer */}
              <div className="bg-slate-900 border border-slate-700 rounded-xl p-4 relative">
                <div className="absolute -top-3 left-1/2 -translate-x-1/2 bg-slate-800 text-slate-300 text-[14px] px-3 py-0.5 rounded-full font-bold tracking-wider uppercase border border-slate-700 whitespace-nowrap">사용자 브라우저</div>
                <Laptop className="text-slate-400 my-2 mx-auto" size={28} />
                <div className="space-y-2 mt-3">
                  <div className="bg-emerald-500/10 border border-emerald-500/30 rounded p-2">
                    <div className="text-[14px] text-emerald-300 font-bold">✓ HttpOnly Session Cookie</div>
                    <div className="text-[13px] text-slate-400 mt-0.5 font-mono">portal_sid=abc...</div>
                  </div>
                  <div className="bg-rose-500/10 border border-rose-500/30 rounded p-2">
                    <div className="text-[14px] text-rose-300 font-bold">✗ Access Token · Refresh Token 저장 안 함</div>
                    <div className="text-[13px] text-slate-400 mt-0.5">XSS 탈취 위험 차단</div>
                  </div>
                </div>
              </div>
              ...
              <div className="bg-slate-900 border border-purple-500/40 rounded-xl p-4 relative">
                <div className="absolute -top-3 left-1/2 -translate-x-1/2 bg-slate-800 text-purple-300 text-[14px] px-3 py-0.5 rounded-full font-bold tracking-wider uppercase border border-purple-500/40 whitespace-nowrap">포털 백엔드</div>
                ...
                <div className="space-y-2 mt-3">
                  <div className="bg-indigo-500/10 border border-indigo-500/30 rounded p-2">
                    <div className="text-[14px] text-indigo-300 font-bold">Access Token (메모리)</div>
                    <div className="text-[13px] text-slate-400 mt-0.5 font-mono">15분 TTL · 세션 캐시</div>
                  </div>
                  <div className="bg-emerald-500/10 border border-emerald-500/30 rounded p-2">
                    <div className="text-[14px] text-emerald-300 font-bold">Refresh Token (Redis)</div>
                    <div className="text-[13px] text-slate-400 mt-0.5 font-mono">7~30일 · 세션 스토어</div>
                  </div>
                </div>
              </div>
              ...
              <div className="bg-slate-900 border border-indigo-500/40 rounded-xl p-4 relative">
                <div className="absolute -top-3 left-1/2 -translate-x-1/2 bg-slate-800 text-indigo-300 text-[14px] px-3 py-0.5 rounded-full font-bold tracking-wider uppercase border border-indigo-500/40 whitespace-nowrap">IdP 인증 서버</div>
                <ShieldCheck className="text-indigo-400 my-2 mx-auto" size={28} />
                <div className="space-y-2 mt-3">
                  <div className="bg-amber-500/10 border border-amber-500/30 rounded p-2">
                    <div className="text-[14px] text-amber-300 font-bold">Refresh Token 해시 (DB)</div>
                    <div className="text-[13px] text-slate-400 mt-0.5 font-mono">SHA-256(rt) + used flag</div>
                  </div>
                  <div className="bg-sky-500/10 border border-sky-500/30 rounded p-2">
                    <div className="text-[14px] text-sky-300 font-bold">JWT 서명 키 (KMS)</div>
                    <div className="text-[13px] text-slate-400 mt-0.5 font-mono">RSA private key</div>
                  </div>
                </div>
              </div>
            </div>
            ...
            {/* Flow arrows */}
            <div className="mt-5 grid grid-cols-3 gap-4 text-[14px] font-mono">
          ...
          <div className="bg-slate-900 border border-emerald-500/30 rounded-2xl overflow-hidden">
            ...
            <div className="p-5 space-y-5">
              ...
              {/* 쿠키 한 줄 분해 */}
              <div>
                <div className="text-[14px] font-mono font-bold uppercase tracking-widest text-emerald-300 mb-2">실제 발급되는 쿠키 (Set-Cookie 한 줄)</div>
            ...
            Max-Age=86400`} />
                <div className="overflow-x-auto mt-3">
                  <table className="w-full text-left text-[14px] border-collapse text-slate-300">
                    <thead>
              ...
              {/* 세션 ID 값 종류 */}
              <div>
                <div className="text-[14px] font-mono font-bold uppercase tracking-widest text-indigo-300 mb-2">세션 ID 값의 3가지 형태</div>
                <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
                  <div className="bg-slate-950/60 border border-emerald-500/40 rounded-xl p-3">
                    <div className="flex items-center gap-1.5 mb-1.5">
                      <span className="text-[13px] font-mono font-bold bg-emerald-500/20 text-emerald-300 px-1.5 py-0.5 rounded">권장</span>
                      <span className="text-[15px] font-bold text-white">Opaque random</span>
                    </div>
                    <div className="text-[13px] font-mono text-emerald-300 break-all bg-slate-950 rounded px-1.5 py-1 mb-1.5">A1f2eK_kLqz9wQv...</div>
                    <p className="text-[14px] text-slate-400 leading-relaxed">무의미한 랜덤 문자열. 백엔드가 세션 스토어에서 이걸 키로 lookup. 폐기 즉시 가능. <strong className="text-emerald-300">Spring Session · express-session · Django session 기본 방식</strong>.</p>
                  </div>
                  <div className="bg-slate-950/60 border border-amber-500/30 rounded-xl p-3">
                    <div className="flex items-center gap-1.5 mb-1.5">
                      <span className="text-[13px] font-mono font-bold bg-amber-500/20 text-amber-300 px-1.5 py-0.5 rounded">대안</span>
                      <span className="text-[15px] font-bold text-white">JWT (Stateless)</span>
                    </div>
                    <div className="text-[13px] font-mono text-amber-300 break-all bg-slate-950 rounded px-1.5 py-1 mb-1.5">eyJhbGciOiJIUzI1NiIs...</div>
                    <p className="text-[14px] text-slate-400 leading-relaxed">세션 정보를 JWT로 인코딩해 쿠키 자체가 되는 방식. 서버 스토어 불필요. 단점: 폐기 어려움(블랙리스트 필요), 크기 큼.</p>
                  </div>
                  <div className="bg-slate-950/60 border border-indigo-500/30 rounded-xl p-3">
                    <div className="flex items-center gap-1.5 mb-1.5">
                      <span className="text-[13px] font-mono font-bold bg-indigo-500/20 text-indigo-300 px-1.5 py-0.5 rounded">변형</span>
                      <span className="text-[15px] font-bold text-white">Signed ID</span>
                    </div>
                    <div className="text-[13px] font-mono text-indigo-300 break-all bg-slate-950 rounded px-1.5 py-1 mb-1.5">A1f2eK.HMAC-SHA256(...)</div>
                    <p className="text-[14px] text-slate-400 leading-relaxed">Opaque ID + HMAC 서명. ID는 서버 lookup용, 서명은 위변조 방지용.</p>
                  </div>
                </div>
              </div>
              ...
              {/* 백엔드 매핑 */}
              <div>
                <div className="text-[14px] font-mono font-bold uppercase tracking-widest text-purple-300 mb-2">백엔드 세션 스토어 (Redis 예시)</div>
...
TTL: 86400`} />
              ...
              {/* API 호출 흐름 */}
              <div>
                <div className="text-[14px] font-mono font-bold uppercase tracking-widest text-sky-300 mb-2">API 호출 흐름 — 토큰이 절대 브라우저에 노출되지 않는 이유</div>
                <ol className="space-y-1.5 text-[15px] text-slate-300 list-decimal list-inside">
      ...
      >
        <div className="p-6 space-y-6">
          ...
          {/* 의사결정 트리 */}
          <div className="bg-slate-950 border border-slate-800 rounded-xl p-5">
            <div className="text-[14px] font-mono font-bold uppercase tracking-widest text-amber-300 mb-4 text-center">의사결정 트리</div>
            <div className="space-y-3 text-sm">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-3 ml-0 md:ml-6">
                <div className="bg-emerald-500/10 border border-emerald-500/30 rounded-lg p-3">
                  <div className="text-emerald-300 font-bold text-[15px] mb-1">YES — BFF 패턴 ✓</div>
                  <p className="text-[14px] text-slate-400 leading-relaxed">서버 세션 + HttpOnly 세션 쿠키만 브라우저로. 가장 안전.</p>
                </div>
                <div className="bg-amber-500/10 border border-amber-500/30 rounded-lg p-3">
                  <div className="text-amber-300 font-bold text-[15px] mb-1">NO — SPA 환경 ↓</div>
                  <p className="text-[14px] text-slate-400 leading-relaxed">Q2로 진행.</p>
                </div>
              </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-3 ml-0 md:ml-12">
                <div className="bg-emerald-500/10 border border-emerald-500/30 rounded-lg p-3">
                  <div className="text-emerald-300 font-bold text-[15px] mb-1">YES — 🥉 Silent SSO 의존</div>
                  <p className="text-[14px] text-slate-400 leading-relaxed">SPA에 토큰 저장 자체를 안 함. 매 페이지 로드 시 IdP에서 새로 받음.</p>
                </div>
                <div className="bg-amber-500/10 border border-amber-500/30 rounded-lg p-3">
                  <div className="text-amber-300 font-bold text-[15px] mb-1">NO — 토큰을 저장해야 함 ↓</div>
                  <p className="text-[14px] text-slate-400 leading-relaxed">Q3로 진행.</p>
                </div>
              </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-3 ml-0 md:ml-16">
                <div className="bg-indigo-500/10 border border-indigo-500/30 rounded-lg p-3">
                  <div className="text-indigo-300 font-bold text-[15px] mb-1">YES — 🥈 Service Worker 격리</div>
                  <p className="text-[14px] text-slate-400 leading-relaxed">토큰을 SW 스코프에 격리. 메인 JS는 토큰 접근 불가 → 브라우저 안의 BFF.</p>
                </div>
                <div className="bg-purple-500/10 border border-purple-500/30 rounded-lg p-3">
                  <div className="text-purple-300 font-bold text-[15px] mb-1">NO — 🥇 메모리 + HttpOnly RT 쿠키</div>
                  <p className="text-[14px] text-slate-400 leading-relaxed">표준 SPA 패턴. AT는 JS 메모리, RT는 HttpOnly 쿠키. + RTR + CSP 필수.</p>
                </div>
              </div>
              ...
              <div className="mt-5 pt-4 border-t border-slate-800">
                <div className="bg-sky-500/10 border border-sky-500/30 rounded-lg p-3 flex items-start gap-3">
                  <span className="shrink-0 text-sky-300 font-bold text-[15px]">💎 DPoP (가산 옵션)</span>
          ...
          <div>
            <h4 className="text-sm font-bold text-white mb-3 flex items-center gap-2">
              <span className="bg-purple-500/20 text-purple-300 text-[14px] font-mono px-2 py-0.5 rounded">PATTERNS</span>
              SPA에서 쓸 수 있는 4가지 패턴
            </h4>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {/* 1순위 */}
              <div className="bg-slate-950/60 border border-purple-500/40 rounded-xl p-4">
                <div className="flex items-center gap-2 mb-2">
                  <span className="text-[14px] font-mono font-bold bg-purple-500/20 text-purple-300 px-2 py-0.5 rounded">🥇 1순위</span>
                ...
                <ul className="text-[14px] text-slate-400 space-y-1 leading-relaxed">
              ...
              <div className="bg-slate-950/60 border border-indigo-500/40 rounded-xl p-4">
                <div className="flex items-center gap-2 mb-2">
                  <span className="text-[14px] font-mono font-bold bg-indigo-500/20 text-indigo-300 px-2 py-0.5 rounded">🥈 2순위</span>
              ...
              <div className="bg-slate-950/60 border border-emerald-500/40 rounded-xl p-4">
                <div className="flex items-center gap-2 mb-2">
                  <span className="text-[14px] font-mono font-bold bg-emerald-500/20 text-emerald-300 px-2 py-0.5 rounded">🥉 3순위</span>
              ...
              <div className="bg-slate-950/60 border border-sky-500/40 rounded-xl p-4">
                <div className="flex items-center gap-2 mb-2">
                  <span className="text-[14px] font-mono font-bold bg-sky-500/20 text-sky-300 px-2 py-0.5 rounded">💎 BONUS</span>
          ...
          <div className="bg-slate-900 border border-emerald-500/30 rounded-2xl overflow-hidden">
            ...
            <div className="p-5 space-y-5">
              ...
              <div className="bg-slate-950/60 border border-emerald-500/30 rounded-xl p-4">
                <div className="flex items-center gap-2 mb-2">
                  <span className="text-[14px] font-mono font-bold bg-emerald-500/20 text-emerald-300 px-2 py-0.5 rounded">RTR</span>
                ...
                <div className="bg-slate-950 border border-slate-800 rounded p-3 mb-2">
                  <div className="text-[14px] font-mono text-slate-300 leading-relaxed">
              ...
              <div className="bg-slate-950/60 border border-indigo-500/30 rounded-xl p-4">
                <div className="flex items-center gap-2 mb-2">
                  <span className="text-[14px] font-mono font-bold bg-indigo-500/20 text-indigo-300 px-2 py-0.5 rounded">CSP</span>
              ...
              <div className="bg-amber-500/5 border border-amber-500/30 rounded-xl p-4">
                <h5 className="text-[15px] font-bold text-amber-200 mb-2 flex items-center gap-2">
          ...
          <div className="bg-rose-500/5 border border-rose-500/30 rounded-xl p-4">
            ...
            <ul className="space-y-1 text-[15px] text-slate-300">
      ...
      >
        <div className="p-6 space-y-5">
          ...
          <div className="bg-slate-950 border border-slate-800 rounded-xl p-5">
            {/* Time axis labels */}
            <div className="flex justify-between text-[14px] font-mono text-slate-500 mb-2 px-1">
              <span>t=0 (로그인)</span>
              <span>15분</span>
              <span>30분</span>
              <span>45분</span>
            ...
            <div className="mb-2">
              <div className="flex items-center gap-1 mb-1">
                ...
                <span className="text-[14px] font-bold text-indigo-300 uppercase tracking-wider">Access Token · 15분마다 교체</span>
              </div>
              <div className="flex h-6 gap-px relative">
                <div className="flex-1 bg-gradient-to-r from-indigo-500/80 to-indigo-500/40 rounded-l border border-indigo-500/50 flex items-center justify-center text-[13px] font-mono text-white font-bold">#1</div>
                <div className="flex-1 bg-gradient-to-r from-indigo-500/80 to-indigo-500/40 border border-indigo-500/50 flex items-center justify-center text-[13px] font-mono text-white font-bold">#2</div>
                <div className="flex-1 bg-gradient-to-r from-indigo-500/80 to-indigo-500/40 border border-indigo-500/50 flex items-center justify-center text-[13px] font-mono text-white font-bold">#3</div>
                <div className="flex-1 bg-gradient-to-r from-indigo-500/80 to-indigo-500/40 border border-indigo-500/50 flex items-center justify-center text-[13px] font-mono text-white font-bold">#4</div>
                <div className="flex-[6] bg-gradient-to-r from-indigo-500/40 via-indigo-500/20 to-indigo-500/10 border border-indigo-500/30 border-dashed flex items-center justify-center text-[13px] font-mono text-slate-400">... 매 15분 자동 회전 ...</div>
                <div className="flex-1 bg-rose-500/30 rounded-r border border-rose-500/40 flex items-center justify-center text-[13px] font-mono text-rose-200">만료</div>
              </div>
            </div>
            ...
            <div className="mt-3">
              <div className="flex items-center gap-1 mb-1">
                ...
                <span className="text-[14px] font-bold text-emerald-300 uppercase tracking-wider">Refresh Token · 7일 + 사용마다 회전(RTR)</span>
              </div>
              <div className="flex h-6 gap-px relative">
                <div className="flex-1 bg-gradient-to-r from-emerald-500/80 to-emerald-500/60 rounded-l border border-emerald-500/50 flex items-center justify-center text-[13px] font-mono text-white font-bold">#1</div>
                <div className="flex-1 bg-gradient-to-r from-emerald-500/80 to-emerald-500/60 border border-emerald-500/50 flex items-center justify-center text-[13px] font-mono text-white font-bold">#2</div>
                <div className="flex-1 bg-gradient-to-r from-emerald-500/80 to-emerald-500/60 border border-emerald-500/50 flex items-center justify-center text-[13px] font-mono text-white font-bold">#3</div>
                <div className="flex-1 bg-gradient-to-r from-emerald-500/80 to-emerald-500/60 border border-emerald-500/50 flex items-center justify-center text-[13px] font-mono text-white font-bold">#4</div>
                <div className="flex-[6] bg-gradient-to-r from-emerald-500/60 via-emerald-500/40 to-emerald-500/20 border border-emerald-500/30 border-dashed flex items-center justify-center text-[13px] font-mono text-slate-300">... 갱신마다 회전 ...</div>
                <div className="flex-1 bg-rose-500/30 rounded-r border border-rose-500/40 flex items-center justify-center text-[13px] font-mono text-rose-200">재로그인</div>
              </div>
            </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-3 text-xs">
            <div className="bg-slate-950/60 border border-emerald-500/30 rounded-xl p-4 relative">
              <div className="absolute -top-2 left-3 bg-emerald-500 text-white text-[13px] font-bold px-2 py-0.5 rounded">1. 발급</div>
            ...
            <div className="bg-slate-950/60 border border-indigo-500/30 rounded-xl p-4 relative">
              <div className="absolute -top-2 left-3 bg-indigo-500 text-white text-[13px] font-bold px-2 py-0.5 rounded">2. 사용</div>
            ...
            <div className="bg-slate-950/60 border border-amber-500/30 rounded-xl p-4 relative">
              <div className="absolute -top-2 left-3 bg-amber-500 text-white text-[13px] font-bold px-2 py-0.5 rounded">3. 갱신</div>
            ...
            <div className="bg-slate-950/60 border border-rose-500/30 rounded-xl p-4 relative">
              <div className="absolute -top-2 left-3 bg-rose-500 text-white text-[13px] font-bold px-2 py-0.5 rounded">4. 폐기</div>
      ...
      >
        <div className="p-6 space-y-5 text-sm text-slate-300 leading-relaxed">
          ...
          <div className="bg-slate-950 border border-slate-800 rounded-xl p-5 space-y-5">
            {/* Normal chain */}
            <div>
              <div className="text-[14px] font-mono font-bold uppercase tracking-widest text-emerald-300 mb-3">정상 회전 (Token Family Chain)</div>
              <div className="flex items-center gap-2 overflow-x-auto">
                <div className="shrink-0 bg-emerald-500/15 border border-emerald-500/40 rounded-lg px-3 py-2 text-center min-w-[80px]">
                  <div className="text-[14px] font-mono font-bold text-emerald-300">#1</div>
                  <div className="text-[13px] text-slate-400 mt-0.5">used</div>
                </div>
                <ChevronRight className="shrink-0 text-emerald-400" size={18} />
                <div className="shrink-0 bg-emerald-500/15 border border-emerald-500/40 rounded-lg px-3 py-2 text-center min-w-[80px]">
                  <div className="text-[14px] font-mono font-bold text-emerald-300">#2</div>
                  <div className="text-[13px] text-slate-400 mt-0.5">used</div>
                </div>
                <ChevronRight className="shrink-0 text-emerald-400" size={18} />
                <div className="shrink-0 bg-emerald-500/15 border border-emerald-500/40 rounded-lg px-3 py-2 text-center min-w-[80px]">
                  <div className="text-[14px] font-mono font-bold text-emerald-300">#3</div>
                  <div className="text-[13px] text-slate-400 mt-0.5">used</div>
                </div>
                <ChevronRight className="shrink-0 text-emerald-400" size={18} />
                <div className="shrink-0 bg-emerald-500/25 border-2 border-emerald-500 rounded-lg px-3 py-2 text-center min-w-[80px] shadow-lg shadow-emerald-500/20">
                  <div className="text-[14px] font-mono font-bold text-emerald-200">#4</div>
                  <div className="text-[13px] text-emerald-300 mt-0.5 font-bold">active</div>
                </div>
              </div>
              <p className="text-[15px] text-slate-400 mt-2">매 갱신마다 한 단계씩 회전. 이전 Refresh Token은 즉시 used 표시되고 DB에서 사용 불가.</p>
            </div>
            ...
            <div>
              <div className="text-[14px] font-mono font-bold uppercase tracking-widest text-rose-300 mb-3">도난 감지 시나리오 — 공격자가 Refresh Token #2 탈취 후 사용 시도</div>
              <div className="space-y-2.5">
                <div className="flex items-center gap-2 text-[15px]">
                  <span className="shrink-0 w-5 h-5 rounded-full bg-slate-700 text-slate-200 font-bold flex items-center justify-center text-[13px]">1</span>
                  <div className="flex items-center gap-2 flex-wrap">
                    <span className="text-slate-300">정상 사용자:</span>
                    <span className="bg-emerald-500/15 border border-emerald-500/40 rounded px-2 py-0.5 font-mono text-[14px] text-emerald-300">Refresh Token #2</span>
                    <ChevronRight className="text-emerald-400" size={12} />
                    <span className="bg-emerald-500/15 border border-emerald-500/40 rounded px-2 py-0.5 font-mono text-[14px] text-emerald-300">Refresh Token #3 발급</span>
                    <span className="text-emerald-400">✓ OK</span>
                  </div>
                </div>
                <div className="flex items-center gap-2 text-[15px]">
                  <span className="shrink-0 w-5 h-5 rounded-full bg-rose-500/30 text-rose-200 font-bold flex items-center justify-center text-[13px]">2</span>
                  <div className="flex items-center gap-2 flex-wrap">
                    <span className="text-rose-300">공격자(뒤늦게):</span>
                    <span className="bg-rose-500/15 border border-rose-500/40 rounded px-2 py-0.5 font-mono text-[14px] text-rose-300 line-through">Refresh Token #2</span>
                ...
                <div className="flex items-start gap-2 text-[15px]">
                  <span className="shrink-0 w-5 h-5 rounded-full bg-amber-500/30 text-amber-200 font-bold flex items-center justify-center text-[13px]">3</span>
                ...
                <div className="ml-7 flex items-center gap-2 overflow-x-auto">
                  <div className="shrink-0 bg-rose-500/10 border border-rose-500/40 rounded-lg px-3 py-1.5 text-center min-w-[70px] opacity-60">
                    <div className="text-[14px] font-mono font-bold text-rose-300 line-through">#1</div>
                  </div>
                  <div className="shrink-0 bg-rose-500/10 border border-rose-500/40 rounded-lg px-3 py-1.5 text-center min-w-[70px] opacity-60">
                    <div className="text-[14px] font-mono font-bold text-rose-300 line-through">#2</div>
                  </div>
                  <div className="shrink-0 bg-rose-500/10 border border-rose-500/40 rounded-lg px-3 py-1.5 text-center min-w-[70px] opacity-60">
                    <div className="text-[14px] font-mono font-bold text-rose-300 line-through">#3</div>
                  </div>
                  <span className="text-[14px] text-rose-300 font-bold ml-2">+ 사용자 강제 로그아웃 + 보안 알림</span>
                </div>
              </div>
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div className="bg-slate-950 border border-emerald-500/30 rounded-xl p-4">
              <div className="flex items-center gap-2 mb-2">
                <span className="bg-emerald-500/20 text-emerald-300 text-[14px] font-mono font-bold px-2 py-0.5 rounded">정상 흐름</span>
              </div>
            ...
            <div className="bg-slate-950 border border-rose-500/40 rounded-xl p-4">
              <div className="flex items-center gap-2 mb-2">
                <span className="bg-rose-500/20 text-rose-300 text-[14px] font-mono font-bold px-2 py-0.5 rounded">도난 감지 시나리오</span>
              </div>
              <ol className="space-y-1.5 text-xs text-slate-300 list-decimal list-inside">
