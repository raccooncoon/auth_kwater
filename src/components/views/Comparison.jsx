  ...
  return (
    <div className="space-y-10">
      ...
      <div className="relative bg-gradient-to-br from-rose-950/40 via-slate-900 to-slate-950 border border-rose-500/30 rounded-3xl p-8 md:p-10 overflow-hidden">
        ...
        <div className="relative">
          <div className="flex items-center gap-2 mb-3">
            <span className="text-[12px] font-mono font-bold uppercase tracking-widest bg-rose-500/20 text-rose-300 border border-rose-500/30 px-2.5 py-0.5 rounded">왜 별도 IdP가 필요한가</span>
            <span className="text-[12px] font-mono font-bold uppercase tracking-widest bg-amber-500/20 text-amber-300 border border-amber-500/30 px-2.5 py-0.5 rounded">설득 근거 자료</span>
          </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div className="bg-slate-900 border border-emerald-500/30 rounded-xl p-5">
            ...
            <ul className="space-y-1.5 text-[13px] text-slate-300">
              ...
              <li className="flex gap-2"><Check className="shrink-0 text-emerald-400 mt-0.5" size={14} /><span>사용자 마스터 조회 (<code className="text-[11px] text-emerald-300">/api/member</code>)</span></li>
              <li className="flex gap-2"><Check className="shrink-0 text-emerald-400 mt-0.5" size={14} /><span>토큰 검증 (<code className="text-[11px] text-emerald-300">/api/auth/validate</code>)</span></li>
          ...
          <div className="bg-slate-900 border border-rose-500/30 rounded-xl p-5">
            ...
            <ul className="space-y-1.5 text-[13px] text-slate-300">
              <li className="flex gap-2"><X className="shrink-0 text-rose-400 mt-0.5" size={14} /><span>OIDC 표준 흐름 (<code className="text-[11px] text-rose-300">/authorize</code>, PKCE, state, nonce)</span></li>
              ...
              <li className="flex gap-2"><X className="shrink-0 text-rose-400 mt-0.5" size={14} /><span>Step-up 재인증 (<code className="text-[11px] text-rose-300">prompt=login</code>)</span></li>
            </ul>
          </div>
        </div>
          ...
          <table className="w-full text-left text-sm border-collapse">
            <thead>
              <tr className="border-b border-slate-700 text-slate-400">
                <th className="py-3 px-4 font-semibold text-[12px] uppercase tracking-wider">항목</th>
                <th className="py-3 px-4 font-semibold text-[12px] uppercase tracking-wider text-indigo-300">디지털플랫폼 통합인증 서버 (제안)</th>
                <th className="py-3 px-4 font-semibold text-[12px] uppercase tracking-wider text-rose-300">OASIS Portal API (현재 가이드)</th>
              </tr>
            </thead>
      ...
      <Section icon={Scale} title="3. 토큰 검증 트릴레마 — OASIS만 사용 시 빠지는 딜레마" subtitle="3가지 옵션 모두 결함이 있고, OIDC 표준은 이 트릴레마를 깸">
        <div className="overflow-x-auto">
          <table className="w-full text-left text-sm border-collapse">
            <thead>
              <tr className="border-b border-slate-700 text-slate-400">
                <th className="py-3 px-4 font-semibold text-[12px] uppercase tracking-wider">방식</th>
                <th className="py-3 px-4 font-semibold text-[12px] uppercase tracking-wider w-32">비밀키 공유</th>
                <th className="py-3 px-4 font-semibold text-[12px] uppercase tracking-wider w-24">성능</th>
                <th className="py-3 px-4 font-semibold text-[12px] uppercase tracking-wider">보안 약점</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-slate-800">
              <tr>
                ...
                <td className="py-3 px-4 text-[13px] text-rose-300">4개 포털에 비밀키 분산 → 한 곳 유출 = 전체 위조</td>
              </tr>
              <tr>
                <td className="py-3 px-4 text-white font-semibold">B. 매 요청마다 <code className="text-emerald-300 text-[12px]">/api/auth/validate</code> 호출</td>
                <td className="py-3 px-4 text-emerald-300 font-bold">✓ 불필요</td>
                <td className="py-3 px-4 text-rose-300 font-bold">❌ 느림</td>
                <td className="py-3 px-4 text-[13px] text-rose-300">OASIS가 4 포털 SPOF, latency hop, 부하 폭발</td>
              </tr>
              <tr>
                <td className="py-3 px-4 text-white font-semibold">C. /validate + 캐시</td>
                <td className="py-3 px-4 text-emerald-300 font-bold">✓ 불필요</td>
                <td className="py-3 px-4 text-emerald-300 font-bold">✓ 빠름</td>
                <td className="py-3 px-4 text-[13px] text-rose-300">캐시 만료 전 권한 회수 무효 (퇴직자·사고 대응 지연)</td>
              </tr>
              <tr className="bg-emerald-500/5">
                <td className="py-3 px-4 text-emerald-200 font-bold">D. <strong>OIDC RS256 + JWKS</strong> (디지털플랫폼 통합인증 서버)</td>
                <td className="py-3 px-4 text-emerald-300 font-bold">✓ 불필요</td>
                <td className="py-3 px-4 text-emerald-300 font-bold">✓ 빠름</td>
                <td className="py-3 px-4 text-[13px] text-emerald-300 font-semibold">트릴레마 깨짐 — 공개키로 로컬 검증 + JWKS 1시간 캐시 + revoke API로 즉시 폐기</td>
              </tr>
            </tbody>
          </table>
        </div>
      ...
      <div className="bg-gradient-to-br from-indigo-950/40 to-slate-950 border-2 border-indigo-500/40 rounded-2xl p-8">
        ...
        <div className="text-[12px] text-slate-500 font-mono mt-3 pt-3 border-t border-slate-800">
...
function CmpRow({ item, ours, theirs, warn }) {
  return (
    <tr className="hover:bg-slate-900/40">
      <td className="py-2.5 px-4 text-slate-300 font-semibold text-[13px] align-top">{item}</td>
      <td className="py-2.5 px-4 text-[13px] text-emerald-200 align-top">{ours}</td>
      <td className={`py-2.5 px-4 text-[13px] align-top ${warn ? 'text-rose-300' : 'text-slate-400'}`}>{warn && <span className="mr-1">⚠</span>}{theirs}</td>
    </tr>
  );
}
      ...
      <div className="flex items-start gap-3 mb-3">
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-2 mb-1">
            ...
            <span className={`text-[10px] font-mono font-bold uppercase tracking-widest px-1.5 py-0.5 rounded border ${badgeMap[severity]}`}>{sevLabel}</span>
          </div>
...
function CounterRow({ objection, rebut }) {
  return (
    <div className="bg-slate-900 border border-slate-800 rounded-xl p-5">
      <div className="text-[13px] mb-3 flex gap-2">
        <span className="shrink-0 text-rose-400 font-bold">"Q.</span>
        <span className="text-slate-200 italic">{objection}"</span>
      </div>
      <div className="text-[13px] flex gap-2 pl-4 border-l-2 border-emerald-500/40">
