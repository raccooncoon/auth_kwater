            ...
            {[
              {
                ...
                steps: [
                  ...
                  { step: 6, from: 1, to: 2, label: "6. CMP 백엔드 → IdP: 토큰 교환", lineColor: "bg-purple-500/80 group-hover:bg-purple-400 shadow-[0_0_15px_1px] shadow-purple-500/40", arrowColor: "text-purple-400", dotClass: "border-purple-500 text-purple-300", labelClass: "text-purple-300 border-purple-500/30", desc: "CMP 백엔드가 백채널(Back-channel)로 IdP /token 엔드포인트와 통신해 인가 코드를 Access Token + Refresh Token + ID Token으로 교환합니다 (client_secret 포함)." },
                ],
              },
              {
                ...
                steps: [
                  { step: 7, from: 0, to: 2, label: "7. 하위 포털 무상호작용 인가 요청 (Silent)", lineColor: "bg-indigo-500/80 group-hover:bg-indigo-400 shadow-[0_0_15px_1px] shadow-indigo-500/40", arrowColor: "text-indigo-400", dotClass: "border-indigo-500 text-indigo-300", labelClass: "text-indigo-300 border-indigo-500/30", desc: "사용자가 데이터허브 등 하위 포털에 접근. 포털 프론트는 토큰이 없음을 인지하고 IdP /authorize에 prompt=none + state로 Silent 요청을 보냅니다." },
                  { step: 8, from: 2, to: 0, label: "8. IdP → 브라우저: 인가 코드 콜백 리다이렉트", lineColor: "bg-indigo-500/80 group-hover:bg-indigo-400 shadow-[0_0_15px_1px] shadow-indigo-500/40", arrowColor: "text-indigo-400", dotClass: "border-indigo-500 text-indigo-300", labelClass: "text-indigo-300 border-indigo-500/30", desc: "IdP가 SSO 쿠키를 확인하여 사용자 식별 후, UI 노출 없이 하위 포털 콜백 URL로 인가 코드를 포함한 302 리다이렉트를 응답합니다." },
                  { step: 9, from: 0, to: 1, label: "9. 브라우저 → 하위 포털 백엔드: 인가 코드 전달", lineColor: "bg-purple-500/80 group-hover:bg-purple-400 shadow-[0_0_15px_1px] shadow-purple-500/40", arrowColor: "text-purple-400", dotClass: "border-purple-500 text-purple-300", labelClass: "text-purple-300 border-purple-500/30", desc: "하위 포털 프론트가 콜백 URL의 code를 추출하여 자기 백엔드(POST /api/auth/exchange)로 전달합니다." },
                  { step: 10, from: 1, to: 2, label: "10. 하위 포털 백엔드 → IdP: 토큰 교환", lineColor: "bg-emerald-500/80 group-hover:bg-emerald-400 shadow-[0_0_15px_1px] shadow-emerald-500/40", arrowColor: "text-emerald-400", dotClass: "border-emerald-500 text-emerald-300", labelClass: "text-emerald-300 border-emerald-500/30", desc: "하위 포털(예: 데이터허브) 백엔드가 인가 코드를 IdP /token과 교환하여 자기 client_id에 묶인 Access Token + Refresh Token + ID Token을 발급받습니다." },
                ],
              },
              {
