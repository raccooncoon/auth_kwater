            ...
            {[
              {
                ...
                steps: [
                  ...
                  { step: 2, from: 3, to: 0, label: "2. 인증 완료 + 암호화 페이로드 발급", lineColor: "bg-sky-500/80 group-hover:bg-sky-400 shadow-[0_0_15px_1px] shadow-sky-500/40", arrowColor: "text-sky-400", dotClass: "border-sky-500 text-sky-300", labelClass: "text-sky-300 border-sky-500/30", desc: "K-water가 사용자 정보를 암호화한 Fasoo 암호화 페이로드를 응답으로 발급하고, 디지털플랫폼 콜백 URL로 브라우저를 리다이렉트합니다." },
                  ...
                  { step: 4, from: 2, to: 0, label: "4. SSO 쿠키 발급 + 인가 코드 콜백 리다이렉트", lineColor: "bg-emerald-500/80 group-hover:bg-emerald-400 shadow-[0_0_15px_1px] shadow-emerald-500/40", arrowColor: "text-emerald-400", dotClass: "border-emerald-500 text-emerald-300", labelClass: "text-emerald-300 border-emerald-500/30", desc: "IdP가 K-water 페이로드를 복호화하여 SSO 세션 쿠키를 Set-Cookie로 발급하고, 동시에 인가 코드를 포함한 CMP 콜백 URL(예: cmp.kwater.com/callback?code=...&state=...)로 302 리다이렉트합니다." },
                  { step: 5, from: 0, to: 1, label: "5. 브라우저 → CMP 백엔드: 인가 코드 전달", lineColor: "bg-purple-500/80 group-hover:bg-purple-400 shadow-[0_0_15px_1px] shadow-purple-500/40", arrowColor: "text-purple-400", dotClass: "border-purple-500 text-purple-300", labelClass: "text-purple-300 border-purple-500/30", desc: "브라우저가 콜백 URL에 도착하면 CMP 프론트엔드가 쿼리에서 code를 추출하여 자기 백엔드(POST /api/auth/exchange)로 전달합니다. state 검증은 이 시점에 수행." },
                  { step: 6, from: 1, to: 2, label: "6. CMP 백엔드 → IdP: 토큰 교환 요청", lineColor: "bg-purple-500/80 group-hover:bg-purple-400 shadow-[0_0_15px_1px] shadow-purple-500/40", arrowColor: "text-purple-400", dotClass: "border-purple-500 text-purple-300", labelClass: "text-purple-300 border-purple-500/30", desc: "CMP 백엔드가 백채널(Back-channel)로 IdP /token 엔드포인트에 grant_type=authorization_code + code + code_verifier + client_secret을 전송합니다." },
                  { step: 7, from: 2, to: 1, label: "7. IdP → CMP 백엔드: 토큰 응답 (AT + RT + ID Token)", lineColor: "bg-emerald-500/80 group-hover:bg-emerald-400 shadow-[0_0_15px_1px] shadow-emerald-500/40", arrowColor: "text-emerald-400", dotClass: "border-emerald-500 text-emerald-300", labelClass: "text-emerald-300 border-emerald-500/30", desc: "IdP가 인가 코드와 PKCE를 검증한 뒤 Access Token + Refresh Token + ID Token을 JSON으로 응답합니다. CMP 백엔드는 이를 서버 세션 스토어에 저장하고 브라우저에는 세션 쿠키만 내려줍니다." },
                ],
              },
              {
                ...
                steps: [
                  { step: 8, from: 0, to: 2, label: "8. 하위 포털 무상호작용 인가 요청 (Silent)", lineColor: "bg-indigo-500/80 group-hover:bg-indigo-400 shadow-[0_0_15px_1px] shadow-indigo-500/40", arrowColor: "text-indigo-400", dotClass: "border-indigo-500 text-indigo-300", labelClass: "text-indigo-300 border-indigo-500/30", desc: "사용자가 데이터허브 등 하위 포털에 접근. 포털 프론트는 토큰이 없음을 인지하고 IdP /authorize에 prompt=none + state로 Silent 요청을 보냅니다." },
                  { step: 9, from: 2, to: 0, label: "9. IdP → 브라우저: 인가 코드 콜백 리다이렉트", lineColor: "bg-indigo-500/80 group-hover:bg-indigo-400 shadow-[0_0_15px_1px] shadow-indigo-500/40", arrowColor: "text-indigo-400", dotClass: "border-indigo-500 text-indigo-300", labelClass: "text-indigo-300 border-indigo-500/30", desc: "IdP가 SSO 쿠키를 확인하여 사용자 식별 후, UI 노출 없이 하위 포털 콜백 URL로 인가 코드를 포함한 302 리다이렉트를 응답합니다." },
                  { step: 10, from: 0, to: 1, label: "10. 브라우저 → 하위 포털 백엔드: 인가 코드 전달", lineColor: "bg-purple-500/80 group-hover:bg-purple-400 shadow-[0_0_15px_1px] shadow-purple-500/40", arrowColor: "text-purple-400", dotClass: "border-purple-500 text-purple-300", labelClass: "text-purple-300 border-purple-500/30", desc: "하위 포털 프론트가 콜백 URL의 code를 추출하여 자기 백엔드(POST /api/auth/exchange)로 전달합니다." },
                  { step: 11, from: 1, to: 2, label: "11. 하위 포털 백엔드 → IdP: 토큰 교환 요청", lineColor: "bg-purple-500/80 group-hover:bg-purple-400 shadow-[0_0_15px_1px] shadow-purple-500/40", arrowColor: "text-purple-400", dotClass: "border-purple-500 text-purple-300", labelClass: "text-purple-300 border-purple-500/30", desc: "하위 포털(예: 데이터허브) 백엔드가 IdP /token에 grant_type=authorization_code + code + code_verifier + client_secret을 전송하여 토큰 교환을 요청합니다." },
                  { step: 12, from: 2, to: 1, label: "12. IdP → 하위 포털 백엔드: 토큰 응답 (AT + RT + ID Token)", lineColor: "bg-emerald-500/80 group-hover:bg-emerald-400 shadow-[0_0_15px_1px] shadow-emerald-500/40", arrowColor: "text-emerald-400", dotClass: "border-emerald-500 text-emerald-300", labelClass: "text-emerald-300 border-emerald-500/30", desc: "IdP가 인가 코드와 PKCE를 검증한 뒤 해당 하위 포털의 client_id에 묶인 Access Token + Refresh Token + ID Token을 응답합니다. 하위 포털 백엔드는 이를 서버 세션 스토어에 저장하고 브라우저에는 BFF 세션 쿠키(예: datahub_session, HttpOnly)만 내려줍니다." },
                ],
              },
              {
